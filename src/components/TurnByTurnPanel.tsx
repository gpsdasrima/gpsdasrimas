...
import { formatStepDistance, type RouteStep } from '../utils/routing';
            ...
            <ul className="max-h-48 overflow-y-auto px-3.5 pb-3">
              {remaining.map((step, i) => (
                <li key={i} className="flex items-center gap-2.5 border-t border-ink-700/60 py-2 first:border-t-0">
                  <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-ink-700 text-sm">
                    {step.icon}
                  </span>
                  <div className="min-w-0 flex-1">
